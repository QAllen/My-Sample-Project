		...
		try {
			...
			while (rs.next()) {
				...
				sb.append(StringFormat.fillRightPadString(
						rs.getString("RECORD_TYPE"), 1, BackInterface.SPACE));// 接口代码，长度为1
				// 对应接口文件字段NO.2
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PART_NUMBER"), 14, BackInterface.SPACE));// 零件SAP号，长度为14
				// 对应接口文件字段NO.3
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PRODUCTION_PLANT"), 4,
						BackInterface.SPACE));// 工厂代码,长度为4
				// 对应接口文件字段NO.4
				sb.append(StringFormat.fillRightPadString(
				...
				// 对应接口文件字段NO.8
				sb.append(StringFormat.fillRightPadString(
						rs.getString("LOCATION"), 4, BackInterface.SPACE));
				// 对应接口文件字段NO.9
				sb.append(StringFormat.fillRightPadString(
						rs.getString("LIFE_CIRCLE"), 1, BackInterface.SPACE));
				// 对应接口文件字段NO.10
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DISPOSITION_CODE"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.11
				sb.append(StringFormat.fillRightPadString(
						rs.getString("END_OF_SUPPLY_TIME"), 4,
						BackInterface.SPACE));// 停供日期,长度4
				// 对应接口文件字段NO.12
				sb.append(StringFormat.fillRightPadString(
						rs.getString("SURFACE_TREAT"), 4, BackInterface.SPACE));// 长度4
				// 对应接口文件字段NO.13
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PREMIUM_CODE"), 3, BackInterface.SPACE));// 长度3
				// 对应接口文件字段NO.14
				sb.append(StringFormat.fillRightPadString(
						rs.getString("FERTIGUNGSORT_S"), 4, BackInterface.SPACE));// company
																					// code
																					// from
																					// EPIS，长度4
				// 对应接口文件字段NO.15
				sb.append(StringFormat.fillRightPadString(
						rs.getString("STATUSSYMBOL_T"), 2, BackInterface.SPACE));
				// 对应接口文件字段NO.16
				sb.append(StringFormat.fillRightPadString(
						rs.getString("GESAMT_LAGERNULL"), 8, "0"));
				// 对应接口文件字段NO.17
				sb.append(StringFormat.fillRightPadString(
						rs.getString("KENNZEICHEN"), 1, BackInterface.SPACE));
				// 对应接口文件字段NO.18
				sb.append(StringFormat.fillRightPadString(
						rs.getString("MATERIAL_GROUP"), 3, BackInterface.SPACE));
				// 对应接口文件字段NO.19
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ENTFALL_W"), 10, "0"));
				// 对应接口文件字段NO.20
				sb.append(StringFormat.fillRightPadString(
						rs.getString("LAGER_BESTAND"), 1, BackInterface.SPACE));
				...
				// 对应接口文件字段NO.32
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DICOUNT_GROUP"), 2, BackInterface.SPACE));
				// 对应接口文件字段NO.33
				sb.append(StringFormat.fillRightPadString(
				...
				// 对应接口文件字段NO.35
				sb.append(StringFormat.fillRightPadString(
						rs.getString("WEIGHT_UNIT"), 3, BackInterface.SPACE));// 重量单位
				// 对应接口文件字段NO.36
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DIMENSION"), 32, BackInterface.SPACE));// 长宽高
				// 对应接口文件字段NO.37
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ESTIMATED_WEIGHT"), 1,
						BackInterface.SPACE));// 是否预估重量
				// 对应接口文件字段NO.38
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ESTIMATED_DIMENSION"), 1,
						BackInterface.SPACE));// 是否预估长度
				// 对应接口文件字段NO.39
				sb.append(StringFormat.fillRightPadString(
						rs.getString("MATERIAL_PLANER"), 3, BackInterface.SPACE));
				// 对应接口文件字段NO.40
				sb.append(StringFormat.fillRightPadString(
						rs.getString("SUBJECT_AREA_PRICING"), 4,
						BackInterface.SPACE));
				// 对应接口文件字段NO.41
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PURCHASER_GROUP"), 3, BackInterface.SPACE));
				// 对应接口文件字段NO.42
				sb.append(StringFormat.fillRightPadString(
						rs.getString("STOCK_ZERO"), 8, "0"));// 零件出零日期
				// 对应接口文件字段NO.43
				sb.append(StringFormat.fillRightPadString(
						rs.getString("COMPOUND_SUPPLIER"), 10,
						BackInterface.SPACE));
				// 对应接口文件字段NO.44
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DANGEROUS_GOODS_INDICATOR"), 3,
						BackInterface.SPACE));
				// 对应接口文件字段NO.45
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ENVIRONMENTALLY_RELEVANT"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.46
				sb.append(StringFormat.fillRightPadString(
						rs.getString("HAZARDOUS_MATERIAL_NUMBE"), 18,
						BackInterface.SPACE));
				// 对应接口文件字段NO.47
				sb.append(StringFormat.fillRightPadString(
						rs.getString("MINIMUM_REMAINING_SHELF_LIFE"), 4, "0"));
				// 对应接口文件字段NO.48
				sb.append(StringFormat.fillRightPadString(
						rs.getString("SHELF_LIFE_EXPIRATION_DATE"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.49
				sb.append(StringFormat.fillRightPadString(
						rs.getString("CALCULATION_OF_SLED"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.50
				sb.append(StringFormat.fillRightPadString(
						rs.getString("STORAGE_PERCENTAGE"), 3, "0"));
				// 对应接口文件字段NO.51
				sb.append(StringFormat.fillRightPadString(
						rs.getString("TOTAL_SHELF_LIFE"), 4, "0"));
				// 对应接口文件字段NO.52
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DANGER_GOOD_KEY"), 3, BackInterface.SPACE));
				// 对应接口文件字段NO.53
				sb.append(StringFormat.fillRightPadString(
						rs.getString("MAXIMUM_STORAGE_PERIOD"), 5, "0"));
				// 对应接口文件字段NO.54
				sb.append(StringFormat.fillRightPadString(
						rs.getString("UNIT_MAXIMUM_STORAGE_PERIOD"), 3,
						BackInterface.SPACE));
				// 对应接口文件字段NO.55
				sb.append(StringFormat.fillRightPadString(
						rs.getString("POSTPONED_SPARE_STARTING_DATE"), 8, "0"));
				// 对应接口文件字段NO.56
				sb.append(StringFormat.fillRightPadString(
						rs.getString("LOCK_OF_AVAILABILITY_CHECK"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.57
				sb.append(StringFormat.fillRightPadString(
						rs.getString("NUMBER_X_PART"), 18, BackInterface.SPACE));
				// 对应接口文件字段NO.58
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ITEM_TYPE"), 4, BackInterface.SPACE));
				// 对应接口文件字段NO.59
				sb.append(StringFormat.fillRightPadString(
						rs.getString("COMPONENT_TYPE"), 3, BackInterface.SPACE));
				// 对应接口文件字段NO.60
				sb.append(StringFormat.fillRightPadString(
						rs.getString("LIMITED_STORING_TIME"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.61
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DANGEROUS_GOODS"), 1, BackInterface.SPACE));
				// 对应接口文件字段NO.62
				sb.append(StringFormat.fillRightPadString(
						rs.getString("CLASS_OF_FIRE_DANGER"), 2,
						BackInterface.SPACE));
				// 对应接口文件字段NO.63
				sb.append(StringFormat.fillRightPadString(
						rs.getString("BALL_BEARING_FLAG"), 1,
						BackInterface.SPACE));
				// 对应接口文件字段NO.64
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PACKAGING_NUMBER"), 10,
						BackInterface.SPACE));
				// 对应接口文件字段NO.65
				sb.append(StringFormat.fillRightPadString(
						rs.getString("MANUFACTURER_PART_NO"), 40,
						BackInterface.SPACE));
				// 对应接口文件字段NO.66
				sb.append(StringFormat.fillRightPadString(
						rs.getString("PRODUCT_HIERARCHIE"), 18,
						BackInterface.SPACE));// 格式化的HTG
				// 对应接口文件字段NO.67
				sb.append(StringFormat.fillRightPadString(
						rs.getString("DISPOCODE"), 1, BackInterface.SPACE));
				// 对应接口文件字段NO.68
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ID_FOR_PRODUCT"), 1, BackInterface.SPACE));
				// 对应接口文件字段NO.69
				sb.append(StringFormat.fillRightPadString(
						rs.getString("ALTERNATIVE_PARTNO"), 18,
						BackInterface.SPACE));
...
//			modifyIncreamPartCode(conn);//删除增量表数据
	...
	public void addFileHistory(String ver_no, String fileName) {
		Connection conn = null;
		PreparedStatement ps = null;
		String sql = "insert into ti_back_interface_file"
				+ "  (id, file_name, ver_no, create_by, create_date)"
				+ " values"
				+ "  (SEQ_TI_BACK_INTERFACE_FILE.Nextval, ?, ?, ?, sysdate)";
		try {
			conn = JdbcUtil.getConnection();
			conn.setAutoCommit(false);
			ps = conn.prepareStatement(sql);
