...
//        		String sql = "select part_sapcode from tt_part_base tpb where not exists (select 1 from ti_epis_backinterface_current tebc where tebc.part_number = tpb.part_sapcode) and status =1 and  exists(select 1 from tt_part_supp a where a.part_id=tpb.part_id) and CLS_ID is not null";
            ...
            public List<TiEpisBackinterfaceCurrent> getModifyPartCode() {
            	...
            	while(rs.next()){
            		...
            		String kind_of_material = StringUtils.defaultString(rs.getString("kind_of_material"));//零件种类HT KT
...
//				List<TiEpisBackinterfaceCurrent> partcodeList = pccd.getModifyPartCode(conn);//比较零件字段并更新零件,返回记录有更改记录的零件号
